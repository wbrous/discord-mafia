...
class MafiaSheduler:
        ...
        async def task():
            ...
            if not await self.start_game():
                ...
                if self.attempts >= 3:
                    ...
                    await self.abstractor.on_message(
                        True
                    )  # God idek what this does but I implemented it so I should know...
    ...
    async def start_game(self):
        ...
        """
        if len(self.abstractor.players) < 5:
            return False
        ...
        assert isinstance(
            maybe_channel, discord.TextChannel
        )  # PYREX NOTE: TEXTUAL_CHANNEL is not specific enough; Thread doesn't have overwrites_for
        ...
        try:
            ...
            for player in self.game.players:
                ...
                if isinstance(user, discord.Member):
                    ...
                    # hmmmm
                    logger.warning(
                        f"User is of type {type(user)}?!?!?!?!?!??!?! What is this?!?!?!??!?!?!"
                    )

            mafia_chat = await channel.create_thread(
            ...
            await mafia_chat.send(
                embed=discord.Embed(
                    ...
                    + "\n".join(
                        [f"- {p.name}" for p in self.game.players if p.role == MAFIA]
                    ),
                )
            )
            ...
            roles = discord.Embed(colour=discord.Colour.blurple(), title="Roles")

            for alignment in Alignment:
                aligned_players = [
                    f"{p.role.emoji} {p.name}: {p.role}"
                    for p in self.game.players
                    if p.role.alignment == alignment
                ]
                if aligned_players:
                    roles.add_field(
                        name=alignment.value,
                        value="\n".join(aligned_players),
                        inline=False,
                    )
            ...
            # After the game, explicitly allow everyone to chat again.
            # During the game we locked @everyone; now we ensure basic messaging
            # permissions are re-enabled so players can chat freely after the game.
            ...
            try:
                ...
                # If we couldn't set the explicit allow due to a permissions/API error,
                # report the error to the channel using the repo's existing error message style.
                error = traceback.format_exc()
                try:
                    await self.message.channel.send(
